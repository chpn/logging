 ...
 * Other than that, the two implementations are very similar in functionality.
 *
...
public class QueueIntFlip {
	...
	public QueueIntFlip( int capacity ) {
		...
		this.elements = new int[ capacity ]; // todo get from
							// TypeAllocator ?
	}
	...
	public int put( int[] newElements , int length) {
		...
		if ( !flipped ) {
			...
			if ( length <= capacity - writePos ) {
				...
				// new elements must be divided between top and
				// bottom of elements array
	...
	public int take( int[] into , int length) {
		int intoWritePos = 0;
		if ( !flipped ) {
			// writePos higher than readPos - available section is
			// writePos - readPos
			...
			if ( length <= capacity - readPos ) {
				// length is lower than the elements available
				// at the top of the elements array - copy
				// directly
				...
				// length is higher than elements available at
				// the top of the elements array
				// split copy into a copy from both top and
				// bottom of elements array.
